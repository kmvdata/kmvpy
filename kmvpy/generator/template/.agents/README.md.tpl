...
| --- | --- | --- | --- |
| `py-dev` | `__PY_PROJECT_NAME__` 后端 API、DTO、router、service、hub、配置、业务异常、用户/管理端 Socket.IO 和冒烟测试基础规则。 | 修改后端接口、配置、业务异常、Socket.IO、跨层边界，或评审后端实现时使用。 | `新增管理端系统设置接口，补 DTO、service、错误码和冒烟测试` |
...
| `fullstack-qa` | 验证与测试：后端 pytest YAML 冒烟、前端 bun lint/build/typecheck、skill frontmatter/译文/README 覆盖和过时路径搜索。 | 交付前验证代码或 skill 文件自身，或规划该跑哪些检查时使用。 | `验证本次 skill 矩阵更新，检查 frontmatter、译文和 README 覆盖` |
